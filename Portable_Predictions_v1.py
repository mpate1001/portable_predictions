    ...
    if DATABASE_AVAILABLE:
        ...
        if engine:
            try:
                ...
                st.success(
                    f"Loaded from database: {len(housing_df):,} housing records, {len(crime_df):,} crime records")
    ...
    with col1:
        ...
        if selected_zip and selected_zip != "No ZIP codes available":
            ...
            if len(zip_data) > 0:
                ...
                st.info(
                    f"**ZIP {selected_zip}** | {primary_city} | {property_count} properties | Avg: ${avg_price:,.0f}")
...
def display_data_explorer(datasets):
    ...
    crime_viz = datasets['crime'].nlargest(10, 'safety_score')[
        ['county_clean', 'safety_score', 'violent_rate', 'property_rate']]
...
def display_model_insights(model_data):
    ...
    if 'database_info' in metadata:
        ...
        st.info(
            f"**Models trained from Database**: {db_info.get('database', 'Unknown')} on {db_info.get('host', 'Unknown')}")
    else:
        ...
        with pred_col1:
            ...
            for name, model in model_data['models'].items():
                ...
                predictions_data.extend(
                    [{'Model': name, 'Prediction': pred} for pred in preds[:100]])  # Sample for performance
...
def display_full_investment_analysis(predictions, purchase_price, crime_data, county):
    ...
    investment_score, score_breakdown = calculate_investment_score(predictions, purchase_price, crime_data,
                                                                   market_context)
...
def main():
    ...
    st.markdown('<div class="sub-header">Advanced Housing Investment Analysis with ML & Crime Data Integration</div>',
                unsafe_allow_html=True)
